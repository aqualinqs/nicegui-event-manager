    ...
    with ui.element("div").classes("w-full h-screen flex flex-row m-0 p-0 gap-0"):
        ...
        with ui.column().classes('w-1/2 h-full bg-white flex flex-col justify-center items-center p-4'):
            with ui.row().classes('items-center'):
                ui.label('MEST').classes('text-2xl font-bold text-gray-800 mb-8')
                ui.label('EV Manager').classes('text-2xl font-bold text-orange-600 mb-8')

            ui.label('Sign In to MEST EV Manager').classes('text-3xl font-bold text-gray-800 mb-8')
